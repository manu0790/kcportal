...
String keywordsInput = HtmlUtil.escape(keywords.replaceAll("\"", "").replace("*",""))+"OOOOO";
...
<aui:form action="<%= searchURL %>" method="get" name="fm" onSubmit="event.preventDefault();">
	...
	<aui:fieldset id="searchContainer">
		...
		<aui:input autoFocus="<%= windowState.equals(WindowState.MAXIMIZED) %>" inlineField="<%= true %>" label="" name="keywords" size="30" value="<%=keywordsInput%>"/>

		<aui:input inlineField="<%= true %>" label="" name="search" src='<%= themeDisplay.getPathThemeImages() + "/common/search.png" %>' title="ok" type="image" />
