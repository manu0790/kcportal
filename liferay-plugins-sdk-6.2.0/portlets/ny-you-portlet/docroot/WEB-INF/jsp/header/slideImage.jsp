...
<div class="existing-images clearfix" style="padding: 15px;">
	<div class="pull-left" style="margin-right: 15px;"> 
		<img id="<portlet:namespace/>current_pic" src="<%=themeDisplay.getPathThemeImages()+"/"+Constant.CURRENT_OWNER+"/"+Constant.LESSON_CARD_PLACEHOLDER%>" style="height: 230px; width:570px;"></img>
	</div>
	...
	<div style="margin-top:238px;">
		...
		<div>
			<span onclick="<portlet:namespace/>saveCropRegion()" class="btn btn-primary normal trigger-save"><liferay-ui:message key="save" /></span>
...
<div class="clearfix">
	...
	<div class="pull-left">
		<h4 class="hide" style="margin-top:0; margin-left:14px;" id="croppedImageHead"> <liferay-ui:message key="preview" /> </h4>
		<div class="hide" id="croppedImage" style="margin-bottom:15px; margin-left:14px;"></div>
	</div>
</div>
    <script type="text/javascript"> 
    var <portlet:namespace />updatedUrl='';
    function closeUploadImage(){
    	 Liferay.Util.getOpener().<portlet:namespace />closeUploadImagePopup('uploadImagepopup',<portlet:namespace />updatedUrl);
    }
    ...
    function <portlet:namespace />handleFileSelect(e){
    	...
    	if(file != null && file != ''){
    		var fileExt = file.trim().split('.');
    		fileExt = fileExt[fileExt.length - 1];
    		var supportedImageExtension = ["jpeg","jpg","bmp","png"];
    		if(supportedImageExtension.indexOf(fileExt.toLowerCase()) == -1) {
    			$('#emptyAndFormatFileErr').html('&nbsp;&nbsp;&nbsp;&nbsp;<liferay-ui:message key="please-select-image-only" />(JPG,JPEG,PNG,BMP)...');
