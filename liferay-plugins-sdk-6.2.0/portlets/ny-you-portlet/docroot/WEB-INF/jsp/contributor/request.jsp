...
long request_targetPlId = LayoutLocalServiceUtil.getFriendlyURLLayout(themeDisplay.getScopeGroupId(), layout.isPrivateLayout(), Constant.PAGE_REQUEST_LESSON).getPlid();
PortletURL requestDetailURL=PortletURLFactoryUtil.create(request, request_portlet_Id, request_targetPlId, PortletRequest.RENDER_PHASE);
requestDetailURL.setParameter(Constant.COMMON_STRING_CONSTANT_ACTION, Constant.REQUEST_DETAILS);

long contributorUserId=(Long)request.getAttribute(Constant.CONTRIBUTOR_USER_ID);
ResourceURL deleteRequestURL=PortletURLFactoryUtil.create(request, request_portlet_Id, request_targetPlId , PortletRequest.RESOURCE_PHASE);
deleteRequestURL.setResourceID("deleteRequest");
deleteRequestURL.setParameter(Constant.COMMON_STRING_CONSTANT_USER_ID,String.valueOf(contributorUserId));
%>
<div id="allRequests" class="tab-pane">
	<div id="requestForInfo">
		<p ng-show="allJsonRequests.length==0"><liferay-ui:message key="no-requests-found" />.</p>
					...
					<div class="span9">
						...
						<BR><p class="signature">     
							...
							<span ng-if=request.answerCount!=0 class="label label-success">
							 {{request.answerCount}} <liferay-ui:message key="answers" /> 
							</span>
						...
						<div class="span3 text-center hidden-phone" ng-if=request.answerType=='req_Lesson'> 
							...
							<small><liferay-ui:message key="request-for-lesson" /></small></p>
						</div>
						...
						<div class="span3 text-center hidden-phone" ng-if=request.answerType=='req_Information'> 
							...
							<small><liferay-ui:message key="request-for-information" /></small></p>
						</div>
						...
						<div class="span3 text-center hidden-phone" ng-if=request.answerType=='req_Opinion'> 
							...
							<small><liferay-ui:message key="request-for-opinion" /></small></p>
						</div>
						...
						<div class="span3 text-center hidden-phone" ng-if=request.answerType=='req_Collaborator'> 
							...
							<small><liferay-ui:message key="request-for-collaboration" /></small></p>
						</div>
...
<div class="pagination pull-right" ng-if="items.length>0" ng-cloak>
        <ul >
          <li ng-class="prevPageDisabled()">
            <a href ng-click="prevPage()">« <liferay-ui:message key="prev" /></a>
          </li>
          <li ng-class="nextPageDisabled()">
            <a href ng-click="nextPage()"><liferay-ui:message key="next" /> »</a>
          </li>
        </ul>
</div>
