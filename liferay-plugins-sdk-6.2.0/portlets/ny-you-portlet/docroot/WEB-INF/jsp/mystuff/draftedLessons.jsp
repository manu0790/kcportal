...
<div> 
	...
	<div class="filter-form row-fluid">
	   ...
	   <div class="row-fluid">
	        <p ng-show="draftedFilteredItems.length==0" ng-cloak>
	        	<liferay-ui:message key="no-lessons-found" />.
	        </p>
			<div class="thumbnail-lessons span4" ng-repeat="lesson in draftedFilteredItems = (lessons | filter:categoryId |filter:{status:'draft'} | filter:tagId | limitTo:limit)" ng-class="($index % 3 == 0) ? 'no-margin clearfix' : ''" style="position: relative" ng-cloak my-stuff-my-lessons-directive>
			<div class="my-flip">
				<div class="back">
				...
				<article class="lesson shadow">
					...
					<span ng-if=lesson.status=='draft'>
					<span>
							<a class="nyu-tooltip cursor-pointer trash-btn btn" onmouseover="tooltipOver(this)" onmouseleave="tooltipOut(this)" data-title='<liferay-ui:message key="delete-lesson" />'  ng-click="deleteLesson('<%=deleteLessonURL%>', lesson.lessonId)"> 
