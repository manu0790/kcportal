...
<h1> <img src="<%= Constant.COMMON_THEME_IMAGE_PATH + Constant.VENDOR_NAME + StringPool.SLASH %>Followers.png"> 
...
<div class="right-section"> 
		...
		<c:if test="${empty myFollowersList}">
		<div class="row" id="noFollowers"><p> 
		<liferay-ui:message key="no-followers-found" />.</p> </div>
		</c:if>
		<c:forEach items="${myFollowersList}" var="myFollowersUser"
			varStatus="i">
			<c:set var="count" value="${count + 1}" scope="page" />
			<c:set var="intcount" value="${count}" scope="page" />
			<c:set var="loopCount" value="${loopCount+1}" scope="page" />
			<c:set var="authorId">${myFollowersUser.userId1}</c:set>
			<c:set var="uploadedTime" value="${myFollowersUser.createDate}"
				scope="page" />
	...
	<c:set var="uploadedTime" value="${lesson.uploadedTime}" scope="page" />	 --%>
			<%
					...
					userDetailsUrl.setParameter(Constant.FROM, "myFollowers");
			
					long authorId = Long.parseLong((String) pageContext
							.getAttribute(Constant.AUTHOR_ID));
					int appreciateCount = CommonUtil
							.getAuthorAppreciateCount(authorId);
					...
					//Date d = new Date(tmp);
					//Date uploadedTime=new Date(pageContext.getAttribute(Constant.USER_GROUP_UPLOAD_TIMES_VAR).toString());
					//Date uploadedTime = (Date) pageContext.getAttribute(Constant.USER_GROUP_UPLOAD_TIMES_VAR);
					//Date uploadedTime = (Date) pageContext.getAttribute(Constant.USER_GROUP_UPLOAD_TIMES_VAR);
			...
			<div class="span4">
				<article class="lesson shadow">
					<div class="clearfix follwer-details">
						<div class="span3">
							<img src="<%=authorImg%>" class="img-circle" width="100">
						</div>
						<div class="content span9">
							<%-- <a href="<%=showAuthorLessonURL+"&"+portletNamespace+"authorId="+authorId%>"> --%>
							<%-- <h4 class="text-overFlow">
							<%=UserLocalServiceUtil.getUser(authorId).getFirstName()%>
							<%=UserLocalServiceUtil.getUser(authorId).getLastName()%>
							</h4> --%>
					...
					<footer>
						<div class="infos">
							<div class="likes">
								<span aria-hidden="true" class="icon-thumbs-up"></span> &nbsp;<%=appreciateCount%><span
									class="sr-only">
										<liferay-ui:message key="likes" />
										</span>
							</div>
							<div class="views">
								<span class="icon-eye-open" aria-hidden="true"></span> &nbsp;<%=viewCount%><span
									class="sr-only">
