...
public class VirtualHostFilter extends BasePortalFilter {
	...
	protected boolean isDocumentFriendlyURL(HttpServletRequest request,
			long groupId, String friendlyURL) throws PortalException,
			SystemException {
		...
		if (friendlyURL.startsWith(_PATH_DOCUMENTS)
			...
			if (pathArray.length == 2) {
				try {
					LayoutLocalServiceUtil.getFriendlyURLLayout(groupId, false,
							friendlyURL);
	...
	protected void processFilter(HttpServletRequest request,
		...
		friendlyURL = StringUtil.replace(friendlyURL, StringPool.DOUBLE_SLASH,
				StringPool.SLASH);
		...
		for (String languageId : languageIds) {
			if (StringUtil.startsWith(friendlyURL, languageId)) {
				...
				if (((pos != -1) && (pos != languageId.length()))
						|| ((pos == -1) && !StringUtil.equalsIgnoreCase(
		...
		friendlyURL = StringUtil.replace(friendlyURL,
				PropsValues.WIDGET_SERVLET_MAPPING, StringPool.BLANK);
		...
		if (!friendlyURL.equals(StringPool.SLASH)
			...
			processFilter(VirtualHostFilter.class, request, response,
					filterChain);

			return;
		}

		LayoutSet layoutSet = (LayoutSet) request
		...
		try {
			LastPath lastPath = new LastPath(contextPath, friendlyURL,
			...
			if (originalFriendlyURL
					.startsWith(PropsValues.WIDGET_SERVLET_MAPPING)) {

				forwardURL.append(PropsValues.WIDGET_SERVLET_MAPPING);

				friendlyURL = StringUtil.replaceFirst(friendlyURL,
						PropsValues.WIDGET_SERVLET_MAPPING, StringPool.BLANK);
			}

			long plid = PortalUtil.getPlidFromFriendlyURL(companyId,
					friendlyURL);
				...
				if (group.isGuest() && friendlyURL.equals(StringPool.SLASH)
					...
					if (layoutSet.isPrivateLayout()) {
						if (group.isUser()) {
							...
							if (isDocumentFriendlyURL(request,
									group.getGroupId(), friendlyURL)) {

								processFilter(VirtualHostFilter.class, request,
										response, filterChain);

								return;
							} else {
								forwardURL
										.append(_PRIVATE_GROUP_SERVLET_MAPPING);
							}
						}
					} else {
						if (isDocumentFriendlyURL(request, group.getGroupId(),
								friendlyURL)) {
			...
			RequestDispatcher requestDispatcher = _servletContext
					.getRequestDispatcher(forwardURL.toString());
	...
	private static final String _PATH_MODULE_SLASH = Portal.PATH_MODULE
			+ StringPool.SLASH;
